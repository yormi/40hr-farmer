...
# Survives terminal exits. Use this instead of dev-server.sh for long sessions.
#
# Start:  ./scripts/dev-server-daemon.sh
# Stop:   ./scripts/dev-server-daemon.sh stop
# Status: ./scripts/dev-server-daemon.sh status
# Logs:   journalctl --user -u 40hr-devserver -f
...
case "${1:-start}" in
  start)
    ...
    systemd-run --user --unit="${UNIT}" \
      ...
      --description="40hr Farmer live-server" \
      "${REPO_DIR}/scripts/dev-server.sh"
    sleep 5
    if curl -sI http://localhost:8888/ | head -1 | grep -q 200; then
